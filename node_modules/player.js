...
Player.getAll= function(cos, util, cb) {
    ...
    cmUtils.getCustomObjectTypeId(cos, util, dbName, function(err, typeId) {
      ...
      async.each(data, function(player, cb) {
        ...
        cos.fetchChildren(player, options, typeId.toString(), function(err, object) {
          // If succesfull, the player object is overwritten with the result,
          // that includes the child objects.
          cb(err);
        });
      }, function(err) {
        cb(err, data);
